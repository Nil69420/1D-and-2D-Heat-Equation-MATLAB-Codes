...
%%% T = temperature = f(x,t)
%%% time derivative = second partial derivative
%%%( T(x,t+dt) - T(x,t) ) / dt = ( T(x+dx, t) - 2*T(x,t) + T(x-dx,t) ) /dx^2
...
T_mat(end,:) = 150; %%%The right end of the pipe is 150 degrees - BCs

%%% Integrate using Euler's and FDM
...
%%%[tt,xx] = meshgrid(t_vec,x_vec); mesh(xx,tt,T_mat) xlabel('X coordinate (m)') ylabel('Time (sec)') zlabel('Temperature (F)')
...
TRK4_mat(end,:) = 150; %%%The right end of the pipe is 150 degrees - BCs
